...
${serializeUntrustedInput(request, {
  ...
  const { output } = await executeStructuredGeminiTask({
    models,
  ...
  return validateModelReview(output, request.files, minimumConfidence);
}
...
export function renderReviewBody(result: ReviewResult): string {
  ...
  lines.push("", "_Review is AI-generated. Reply to the bot to ask questions or rate a finding._");
  return lines.join("\n");
}
